    ...
    rpcConsole(0)
{
  ...
  //connect(mainPage->sideBar, SIGNAL(clickedLogoImage()), this, SLOT(aboutClicked()));
    ...
    setWindowTitle(tr("TheGCCcoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
//    setStyleSheet("#stealthWallet { background-color: qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:1, stop:0 #eeeeee, stop:1.0 #fefefe); } QToolTip { color: #000000; background-color: #ffffff;  border:0px;} ");
    setStyleSheet("#stealthWallet { background-color: #eaeaea); } QToolTip { color: #000000; background-color: #ffffff;  border:0px;} ");
...
//    connect(mainPage->pageTransactions->table->table, SIGNAL(doubleClicked(QModelIndex)), this, mainPage->pageTransactions, SLOT(showDetails()));
  ...
  //connect(mainPage, SIGNAL(fileExport()), this, SLOT());
    ...
    //slotGoToMainMenu(); // disable starting main menu
	gotoOverviewPage();  // go to directly to overviewpage in start
}
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a TheGCCcoin address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About TheGCCcoin"), this);
    aboutAction->setToolTip(tr("Show information about TheGCCcoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for TheGCCcoin"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("TheGCCcoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("TheGCCcoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to TheGCCcoin network.", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid TheGCCcoin address or malformed URI parameters."));
    }
