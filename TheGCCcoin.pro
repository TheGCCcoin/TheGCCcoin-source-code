...
# CryptoPP
#LIBS += $$CRYPTO_LIB_PATH/libcryptopp.a
!win32 {
    gencryptopp.commands = cd $$CRYPTO_LIB_PATH && CC=$$QMAKE_CC CXX=$$QMAKE_CXX $(MAKE) OPT=\"$$QMAKE_CXXFLAGS $$QMAKE_CXXFLAGS_RELEASE\" static
} else {
    ...
    LIBS += -lshlwapi
    #gencryptopp.commands = cd $$CRYPTO_LIB_PATH && CC=$$QMAKE_CC CXX=$$QMAKE_CXX TARGET_OS=OS_WINDOWS_CROSSCOMPILE $(MAKE) OPT=\"$$QMAKE_CXXFLAGS $$QMAKE_CXXFLAGS_RELEASE\" static && $$QMAKE_RANLIB $$CRYPTO_LIB_PATH/libcryptopp.a
}
gencryptopp.target = $$CRYPTO_LIB_PATH/libcryptopp.a
gencryptopp.depends = FORCE
PRE_TARGETDEPS += $$CRYPTO_LIB_PATH/libcryptopp.a
...
LIBS += $$join(BOOST_LIB_PATH,,-L,) $$join(BDB_LIB_PATH,,-L,) $$join(OPENSSL_LIB_PATH,,-L,) $$join(QRENCODE_LIB_PATH,,-L,) $$join(LIBEVENT_LIB_PATH,,-L,) $$join(CRYPTO_LIB_PATH,,-L,)
LIBS += -lssl -lcrypto -ldb_cxx$$BDB_LIB_SUFFIX
LIBS +=  -levent -lcryptopp -lz
