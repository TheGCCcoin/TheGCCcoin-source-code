...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, const CCoinControl* coinControl)
{
	...
	//NATURE_SCRIPT << OP_DUP << OP_HASH160 << ParseHex(NATURE_ADDRESS) << OP_EQUALVERIFY << OP_CHECKSIG;
	//int64 natureValue = (nValue * 24 )/ 10000;
	//vecSend.push_back(make_pair(NATURE_SCRIPT,natureValue));
...
std::string CWallet::SendMoney(const std::vector<std::pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, bool fAskFee)
{
    CReserveKey reservekey(this);
    int64 nFeeRequired;
...
std::string CWallet::SendMoneyToDestination(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, bool fAskFee)
{
    int64 nValue = 0;
    for (size_t i = 0; i < vecSend.size(); ++i)
        nValue += vecSend.at(i).second;
