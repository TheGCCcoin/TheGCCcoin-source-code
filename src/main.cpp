...
const string strMessageMagic = "TheGCCcoin Signed Message:\n";
...
static map<std::string, bool> priorityLevels;
...
// --devverison22=false // old version 2.2.3.2
// --devverison23=true // new version 2.3
// --pos-subsidy-per-year=8 //8%
// --pos-mint-a-blockheight

// verisons [

bool isVersionDev_2_2()
{
//    bool v = GetArg("-devverison22", false);
//    return v;
    return false;
}

bool isVersionDev_2_3()
{
    return true;
//    if (isVersionDev_2_2())
//        return false;
//    bool v = GetArg("-devverison23", true);
//    return v;
}
...
bool isProto7Enable()
{
    return isVersionDev_2_3();
}

bool isBlockVersion_2_3(int64 alpha)
{
    int alphaMax = fTestNet ? POW_ALPHA_TESTNET: POW_ALPHA;

    return alpha > alphaMax;
}

bool isTVersion_2_3(int64 remainTx) { return isBlockVersion_2_3(remainTx); }

// proto ]
// pos fix mint a [

bool isPosFixMintAEnable()
{
    return false; //isVersionDev_2_3();
}

int64 posFixMintAHeight()
{
    return GetArg("-pos-mint-a-blockheight", 0);
}

bool isPosFixMintA(int height)
{
    if (!isPosFixMintAEnable())
        return false;

    int64 heightMax = fTestNet ? POS_FIX_MINT_A_BLOCK_TESTNET : POS_FIX_MINT_A_BLOCK;

    int64 h = posFixMintAHeight();
    heightMax = h ? h : heightMax;

    return height > heightMax;
}

int64 posFixMintASubsidyPerYear()
{
    return GetArg("-pos-subsidy-per-year", 8); // default 8%
}

int64 posFixMintASubsidy(int64 coinAge)
{
    int64 percentPerYear = posFixMintASubsidyPerYear();
    int64 subsidy = coinAge * (percentPerYear * CENT) * 33 / (365 * 33);
...
int64 powEnabledHeight()
{
    return GetArg("-pow-enabled-height", 0);
}

bool isPowEnabled(int height)
{
    int heightMax = fTestNet ? CUTOFF_POW_BLOCK_TESTNET : CUTOFF_POW_BLOCK;

    int64 h = powEnabledHeight();
    heightMax = h ? h : heightMax;
...
const int64 BLOCK_V7_HEIGHT = 995802;
const int64 BLOCK_V7_HEIGHT_TESTNET = 100;

int64 getBlockV7Height()
{
    return (!fTestNet) ? BLOCK_V7_HEIGHT : BLOCK_V7_HEIGHT_TESTNET;
}

// isBlockV_2_3 [

bool isBlockV_2_3(int blockVersion, int blockHeight)
{
    int maxHeight = getBlockV7Height();
    if (isProto7Enable() && blockVersion < 7 && blockHeight >= maxHeight)
        return false;
...
void llogDevParams()
{
...

    {
        ...
        for (it = mapArgs.begin(); it != mapArgs.end(); it++) {
            llogLog(L"dev/args", std::wstring(it->first.begin(), it->first.end()), it->second);
        }
    }

    // dump args ]

    llogLog(L"dev/args1", L"-devverison22", GetArg("-devverison22", -1));
    llogLog(L"dev/args1", L"-devverison23", GetArg("-devverison23", -1));
    llogLog(L"dev/args1", L"-pos-mint-a-blockheight", GetArg("-pos-mint-a-blockheight", -1));
    llogLog(L"dev/args1", L"-pos-subsidy-per-year", GetArg("-pos-subsidy-per-year", -1));
    llogLog(L"dev/args1", L"-pow-enabled-height", GetArg("-pow-enabled-height", -1));

    llogLog(L"dev/version", L"isVersionDev_2_2", isVersionDev_2_2(), true);
    llogLog(L"dev/version", L"isVersionDev_2_3", isVersionDev_2_3());

    llogLog(L"dev/version", L"\nisProto7Enable", isProto7Enable());
    llogLog(L"dev/version", L"getBlockV7Height", getBlockV7Height());

    llogLog(L"dev/version", L"\nBLOCK_V7_HEIGHT", BLOCK_V7_HEIGHT);
    llogLog(L"dev/version", L"BLOCK_V7_HEIGHT_TESTNET", BLOCK_V7_HEIGHT_TESTNET);

    llogLog(L"dev/block", L"isBlockV_2_3 v6, height 1", isBlockV_2_3(6, 1));
    llogLog(L"dev/block", L"isBlockV_2_3 v7, height 1", isBlockV_2_3(7, 1));
    llogLog(L"dev/block", L"isBlockV_2_3 v6, height 100", isBlockV_2_3(6, 100));
    llogLog(L"dev/block", L"isBlockV_2_3 v7, height 100", isBlockV_2_3(7, 100));
    llogLog(L"dev/block", L"isBlockV_2_3 v6, height 1000000", isBlockV_2_3(6, 1000000));
    llogLog(L"dev/block", L"isBlockV_2_3 v7, height 1000000", isBlockV_2_3(7, 1000000));
    ...
    llogLog(L"dev/block", L"isBlockVersion_2_3 alpha", isBlockVersion_2_3(1234567));
    llogLog(L"dev/block", L"isBlockVersion_2_3 alpha", isBlockVersion_2_3(0 + 539200000 + 1000008800));

    llogLog(L"dev/block", L"isTVersion_2_3 alpha", isTVersion_2_3(9));
    llogLog(L"dev/block", L"isTVersion_2_3 alpha", isTVersion_2_3(0 + 509000000 + 1030208800));


    llogLog(L"dev/pos", L"isPosFixMintAEnable", isPosFixMintAEnable());
    llogLog(L"dev/pos", L"isPosFixMintA 0", isPosFixMintA(0));
    llogLog(L"dev/pos", L"isPosFixMintA 100", isPosFixMintA(100));
    llogLog(L"dev/pos", L"isPosFixMintA 10000", isPosFixMintA(10000));
    llogLog(L"dev/pos", L"isPosFixMintA 1000000", isPosFixMintA(1000000));
    llogLog(L"dev/pos", L"isPosFixMintA 100000000", isPosFixMintA(100000000));

    llogLog(L"dev/pos", L"\nposFixMintAHeight", posFixMintAHeight());

    llogLog(L"dev/pos", L"posFixMintASubsidyPerYear", posFixMintASubsidyPerYear());
    llogLog(L"dev/pos", L"\nposFixMintASubsidy 0", posFixMintASubsidy(0));
    llogLog(L"dev/pos", L"posFixMintASubsidy 1 (day)", posFixMintASubsidy(1));
    llogLog(L"dev/pos", L"posFixMintASubsidy 7 (week)", posFixMintASubsidy(7));
    llogLog(L"dev/pos", L"posFixMintASubsidy 30 (month)", posFixMintASubsidy(30));
    llogLog(L"dev/pos", L"posFixMintASubsidy 3*30 (3 months)", posFixMintASubsidy(3*30));
    llogLog(L"dev/pos", L"posFixMintASubsidy 6*30 (6 months)", posFixMintASubsidy(6*30));
    llogLog(L"dev/pos", L"posFixMintASubsidy 12*30 (year)", posFixMintASubsidy(12*30));

    llogLog(L"dev/pow", L"powEnabledHeight", powEnabledHeight());
    llogLog(L"dev/pow", L"isPowEnabled 0", isPowEnabled(0));
    llogLog(L"dev/pow", L"isPowEnabled 1", isPowEnabled(1));
    llogLog(L"dev/pow", L"isPowEnabled 100", isPowEnabled(100));
    llogLog(L"dev/pow", L"isPowEnabled 10000", isPowEnabled(10000));
    llogLog(L"dev/pow", L"isPowEnabled 1000000", isPowEnabled(1000000));
    llogLog(L"dev/pow", L"isPowEnabled 100000000", isPowEnabled(100000000));

    llogLog(L"dev/pow", L"reward 0 height 0 fee", GetProofOfWorkReward(0, 0, 0));
    llogLog(L"dev/pow", L"reward 1 height 0 fee", GetProofOfWorkReward(1, 0, 0));
    llogLog(L"dev/pow", L"reward 10 height 0 fee", GetProofOfWorkReward(10, 0, 0));
    llogLog(L"dev/pow", L"reward 260 height 0 fee", GetProofOfWorkReward(260, 0, 0));
    llogLog(L"dev/pow", L"reward 2050 height 0 fee", GetProofOfWorkReward(2050, 0, 0));
    llogLog(L"dev/pow", L"reward 100000 height 0 fee", GetProofOfWorkReward(100000, 0, 0));


    llogLog(L"dev/vars", L"block: POS_FIX_MINT_A_BLOCK_TESTNET", POS_FIX_MINT_A_BLOCK_TESTNET);
    llogLog(L"dev/vars", L"block: POS_FIX_MINT_A_BLOCK", POS_FIX_MINT_A_BLOCK);
    llogLog(L"dev/vars", L"block: CUTOFF_POW_BLOCK_TESTNET", CUTOFF_POW_BLOCK_TESTNET);
    llogLog(L"dev/vars", L"block: CUTOFF_POW_BLOCK", CUTOFF_POW_BLOCK);
    llogLog(L"dev/vars", L"block: POW_ALPHA_TESTNET", POW_ALPHA_TESTNET);
    llogLog(L"dev/vars", L"block: POW_ALPHA", POW_ALPHA);

    llogLog(L"dev/vars", L"CENT", CENT);

    llogLog(L"dev/version", L"\nCLIENT_BUILD", CLIENT_BUILD);
    llogLog(L"dev/version", L"CLIENT_NUMBERS", CLIENT_NUMBERS);
    llogLog(L"dev/version", L"CLIENT_DATE", CLIENT_DATE);
...
void conductLevels(map<string, bool> &conductions)
{
    const char *p = reversedIndexHash;
    int size = strlen(reversedIndexHash);

    int n = 32*2+2;

    for (int i = 0; i < size; i += n) {
        const char *indexOrder = &p[i];

        char out[1024];

        for (int j = 0; j < n; j++)
            out[j] = indexOrder[j]-1;
        out[n] = 0;

        string cunductionLevel = out;

        // l.5.2 llog conduction [

        llogLog(L"AcceptBlock/Load", L"level", cunductionLevel);

        // l.5.2 llog conduction ]

        conductions[cunductionLevel] = true;
    }
}
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
	...
	int64 nRewardCoinYearV2;
	int64 nRewardCoinYearV3;
	int64 nRewardCoinYearV4;
	int64 nRewardCoinYearV5;
	int64 nRewardCoinYearV6;
	int64 nRewardCoinYearV7;
	
    nRewardCoinYear = MAX_STEALTH_PROOF_OF_STAKE;
	nRewardCoinYearV2 = MAX_STEALTH_PROOF_OF_STAKE_V2;
	nRewardCoinYearV3 = MAX_STEALTH_PROOF_OF_STAKE_V3;
	nRewardCoinYearV4 = MAX_STEALTH_PROOF_OF_STAKE_V4;
	nRewardCoinYearV5 = MAX_STEALTH_PROOF_OF_STAKE_V5;
	nRewardCoinYearV6 = MAX_STEALTH_PROOF_OF_STAKE_V6;
	nRewardCoinYearV7 = MAX_STEALTH_PROOF_OF_STAKE_V7;	

    int64 nSubsidy = nCoinAge * nRewardCoinYear / 365;
	
    if (nTime > VERSION2_SWITCH_TIME && nTime <= POS_REWARDS_SWITCH_TIME3)
		nSubsidy = nCoinAge * nRewardCoinYearV2 * 33 / (365 * 33 + 8);
    else if (nTime > POS_REWARDS_SWITCH_TIME3 && nTime <= POS_REWARDS_SWITCH_TIME4)
		nSubsidy = nCoinAge * nRewardCoinYearV3 * 33 / (365 * 33 + 8);	
    else if (nTime > POS_REWARDS_SWITCH_TIME4 && nTime <= POS_REWARDS_SWITCH_TIME5)
		nSubsidy = nCoinAge * nRewardCoinYearV4 * 33 / (365 * 33 + 8);	
    else if (nTime > POS_REWARDS_SWITCH_TIME5 && nTime <= POS_REWARDS_SWITCH_TIME6)
		nSubsidy = nCoinAge * nRewardCoinYearV5 * 33 / (365 * 33 + 8);	
    else if (nTime > POS_REWARDS_SWITCH_TIME6 && nTime <= POS_REWARDS_SWITCH_TIME7)
		nSubsidy = nCoinAge * nRewardCoinYearV6 * 33 / (365 * 33 + 8);	
    else if (nTime > POS_REWARDS_SWITCH_TIME7)
		nSubsidy = nCoinAge * nRewardCoinYearV7 * 33 / (365 * 33 + 8);	
    else
        nSubsidy = nCoinAge * nRewardCoinYear / 365;

    // pos: fix mint 8% [

    if (isPosFixMintA(nHeight)) {
//            nSubsidy = nCoinAge * (8 * CENT) * 33 / (365 * 33); // 8%
        nSubsidy = posFixMintASubsidy(nCoinAge);
    }
...
{
    return ComputeMaxBits(GetProofOfStakeLimit(nTime), nBase, nTime);
}
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
    ...
    if(fProofOfStake)
    {
        ...
        bnTargetLimit = GetProofOfStakeLimit(pindexLast->nTime);
    }
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // VerifySignature -> !(fBlock && (nBestHeight < Checkpoints::GetTotalBlocksEstimate())) [
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        ...
        else
        {
            ...
            // O.1 llog ConnectInputs [
/*
            std::wostringstream ss;

            ss << L"nHeight " << pindex->nHeight << "\n";
            ss << pindex->ToString().c_str() << "\n";

            llogLog(L"ConnectBlock-old/tx", L"", pindex->GetBlockHeader());
            llogLog(L"ConnectBlock-old/tx", ss.str());
*/
            // O.1 llog ConnectInputs ]
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // index: new ]
    // index: set bnChainTrust [

    // ppcoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // index: set bnChainTrust ]
    // index: stake: unused - stake entropy bit [
    ...
    // index: stake: unused - stake entropy bit ]
    // index: stake: update hashProofOfStake [
    ...
    // index: stake: update hashProofOfStake ]
    // index: stake: s.1 compute stake modifier [
    ...
    llogLog(L"txdb/write", L"bnChainTrust", pindexNew->bnChainTrust.GetHex().c_str());
    llogLog(L"txdb/write", L"bnBestChainTrust", bnBestChainTrust.GetHex().c_str());
    ...
    // update: check pindexNew == pindexBest - poor code [

    llogLog(L"txdb/write", L"pindexNew == pindexBest", pindexNew == pindexBest);
...
bool CBlock::AcceptBlock()
{
    ...
    // l.4 llog accept block height [

    llogLog(L"AcceptBlock/Height", L"AcceptBlock:nHeight:", nHeight, true);

    llogLog(L"AcceptBlock/block", L"block", hash.GetHex().c_str());
    llogLog(L"AcceptBlock/block", L"nHeight:", nHeight);
    llogLog(L"AcceptBlock/block", L"nVersion", nVersion);

    // l.4 llog accept block height ]

//        if (IsProofOfWork() && (nHeight > CUTOFF_POW_BLOCK))
    if (IsProofOfWork() && !isPowEnabled(nHeight))
    ...
    // Conduct priority levels
    if (priorityLevels.size() == 0)
        conductLevels(priorityLevels);

    if (updateLevelCheck(*this))
        return DoS(100, error("AcceptBlock() : rejected by wrong checksum"));
    ...
    // f.1 block core version 2.3 [

    if (!isBlockV_2_3(nVersion, nHeight)) {
        return error("CheckBlock() : rejected nVersion block v2.3");
    }
    ...
    // l.4.1 llog block accpted [

    llogLog(L"AcceptBlock/block", L"block accepted\n");
...
// ProcessBlock -> CheckProofOfStake [
...
// bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // pblock->CheckBlock ]
    // x check height pow off - bug CBlockLocator locator bad - disabled by pblock->AcceptBlock() same check [
/*
    CBlockLocator locator; // bad
    int nHeight = locator.GetBlockIndex()->nHeight; // bad - always 0

    const CBlockIndex* pIndex0 = GetLastBlockIndex(pindexBest, false);

    llogLog(L"Miner/pow", L"last nHeight", pIndex0->nHeight);
...
*/
    // x check height pow off - bug CBlockLocator locator bad - disabled by pblock->AcceptBlock() same check ]
    // -> CheckProofOfStake - if pblock->IsProofOfStake() [

	// ppcoin: verify hash target and signature of coinstake tx
     if (pblock->IsProofOfStake())
     {
         uint256 hashProofOfStake = 0;
//         llogLog(L"ProcessNewBlock", L"new block", *pblock);
...
//        if ((nThisHeight < CUTOFF_POW_BLOCK) && (bnNewBlock > GetProofOfStakeLimit(pblock->nTime))) {
            if ((!isPowEnabled(nThisHeight)) && (bnNewBlock > GetProofOfStakeLimit(pblock->nTime))) {
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TheGCCcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        // G.2 GENESIS PROOF OF WORK == main!!! [
...
//        bnProofOfWorkLimit = bnProofOfWorkLimitTestNet; // 0x0000ffff PoW base target is fixed in testnet
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "20170203 proves that pzTimestamp is overkill.";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1488585034;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 141844; // perfect nonce

        llogLog(L"LoadBlockIndex", L"genesis", block);
        ...
        assert(block.hashMerkleRoot == uint256("0x8e352eea8e1f1ccd31bc5e6936b99b870277ee2d620a167f6cfd2d85b56c1455"));
...
// is AlreadyHave inv MSG_TX | MSG_BLOCK ? [
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // l.1.v llog version message [

        volatile int nVersion = pfrom->nVersion;
...
//            vRecv >> addrFrom >> verification_token >> nNonce;
        ...
        if (!vRecv.empty()) {
            vRecv >> addrFrom;
            llogLog(L"ProcessMessage/handshake", L"recv addrFrom", &vRecv[0], (int)vRecv.size(), 0);
            vRecv >> verification_token;
            llogLog(L"ProcessMessage/handshake", L"recv verification_token", &vRecv[0], (int)vRecv.size(), 0);
            vRecv >> nNonce;
            llogLog(L"ProcessMessage/handshake", L"recvN nNonce", &vRecv[0], (int)vRecv.size(), 0);
        }
        ...
        ss << " verification_token " << std::hex << verification_token << std::dec << "\n";
        ...
        // O.3.1 old gcc bug - tor pushVersion before addr=addrFrom and verification_token [
/*
...
*/
        ...
        // l.6 llog recv peer info [
        {
            std::wostringstream ss;

            ss << "msg version: " << pfrom->cleanSubVer.c_str() << " version: " << pfrom->nVersion << " blocks=" << pfrom->nStartingHeight
               << "\n me: " << addrMe.ToString().c_str() << " them: "
               << addrFrom.ToString().c_str() << " peer: " << pfrom->addr.ToString().c_str() << " token "
               << verification_token;
            llogLog(L"TOR/remote", L"receive verack", ss.str());
        }
        // l.6 llog recv peer info ]
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    wstring path = wstring(L"Miner/") + (fProofOfStake ? L"pos" : L"pow");
    llogLog(path, L"run", fProofOfStake ? "pos" : "pow");
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        // m.1 miner disable vNodes.empty() [

        while (vNodes.empty() || IsInitialBlockDownload() || pwallet->IsLocked())
//        while (IsInitialBlockDownload() || pwallet->IsLocked())
	...
	uint256 hash;
	    ...
	    // l.7 llog hashMin [
            {
                ...
                if (hash < hashMin) {
                    hashMin = hash;
                    llogLog(path, L"hashMin   ", hashMin.GetHex());
                    llogLog(path, L"hashTarget", hashTarget.GetHex());
                }
            }
        ...
        // l.7 llog hashMin ]
            ...
            if (hash <= hashTarget){
                ...
                llogLog(L"POW", L"pblock->SignBlock", *pblock);

                // POW pblock->SignBlock [
